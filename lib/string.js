...
//******************************************************************************
  ...
  var __sp = S.prototype = {
    ...
    various: function (opening, closing, delimiter) {
      var s = this.s
      var opening = opening || Export.TMPL_OPEN
      var closing = closing || Export.TMPL_CLOSE
      ...
      var matches = s.match(r) || [];
      matches.forEach(function (match) {
        var replace = match.replace(opening, '').replace(closing, ''),
            various = replace.split(delimiter);
        if (various.length !== 0) {
          various = various[Math.floor(Math.random() * (various.length))];
          s = s.replace(match, various);
        } else {
